...
def merge_adapters(adapters: List[Matrix]) -> Matrix:
    ...
    assert all(num_rows[0] == num_rows[i] for i in range(1, len(num_rows))), "All adapters must have the same number of rows"
...
def create_ada_indices(row_indices, column_indices, ada_row_indices, ada_column_indices, device):
    '''
...
def create_ada_layout(matix:Matrix):
    ...
    layout = torch.ones((matix.size()[0]//block_size, matix.size()[1]//block_size), dtype=torch.int32, device=matix.device) * -1
    blck=0
